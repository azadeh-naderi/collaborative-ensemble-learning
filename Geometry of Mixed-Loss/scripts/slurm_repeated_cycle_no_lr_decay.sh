#!/bin/bash
#SBATCH --job-name=geo-repcycle-noLR
#SBATCH --output=logs/geometry_repcycle_noLR_%A_%a.out
#SBATCH --error=logs/geometry_repcycle_noLR_%A_%a.err
...
# Only K=20 and K=40 are covered here (where the effect was clearest);
# results go to a SEPARATE directory so the existing repeated_cycle data
# is untouched.
...
OUT_DIR="Geometry of Mixed-Loss/results/repeated_cycle_no_lr_decay/k${K}_seed${SEED}"
