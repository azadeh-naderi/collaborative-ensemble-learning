...
with TimeLogger():
    ...
    # New Oracle blocking rule: block only after repeated negative Oracle gains
    oracle_negative_count = torch.zeros(num_models, dtype=torch.long)
    oracle_patience = 2
    oracle_blocked_models = set()
    oracle_block_rounds = {}
    ...
    for round_idx in range(n_rounds):
        ...
        # Remove Oracle pairs for models that are already blocked
        filtered_pairs = []

        for (m1, id1), (m2, id2) in pairs:
            if id1 == oracle_id or id2 == oracle_id:
                student_id = id2 if id1 == oracle_id else id1

                if student_id in oracle_blocked_models:
                    print(f"  skip oracle pair: oracle -> student {student_id} is blocked")
                    continue

            filtered_pairs.append(((m1, id1), (m2, id2)))
        ...
        for (m1, id1), (m2, id2) in pairs:
            ...
            if id1 == oracle_id or id2 == oracle_id:
                ...
                # Patience-based Oracle blocking
                if oracle_gain_student_var < 0:
                    oracle_negative_count[student_id] += 1
                    print(
                        f"  negative oracle gain count for student {student_id}: "
                        f"{oracle_negative_count[student_id].item()}/{oracle_patience}"
                    )
                else:
                    oracle_negative_count[student_id] = 0

                if oracle_negative_count[student_id] >= oracle_patience:
                    oracle_blocked_models.add(student_id)
                    oracle_block_rounds[student_id] = round_idx + 1
                    print(
                        f"  BLOCK oracle for student {student_id}: "
                        f"{oracle_patience} consecutive negative oracle gains "
                        f"by round {round_idx + 1}"
                    )
