...
with TimeLogger():
    ...
    for round_idx in range(n_rounds):
        ...
        print(f"\n=== Round {round_idx + 1}/{n_rounds} ({current_pairing_strategy}) ===")
        ...
        pairs = pairing_methods[current_pairing_strategy]()
        ...
        # =================================================
        # Adaptive switching based on moving average oracle gain
        # =================================================

        oracle_gain_window.append(float(oracle_gain_student_var))

        if len(oracle_gain_window) > window_size:
            oracle_gain_window.pop(0)

        mean_recent_oracle_gain = np.mean(oracle_gain_window)

        print(
            f"Recent oracle gain moving average "
            f"({len(oracle_gain_window)}/{window_size}): "
            f"{mean_recent_oracle_gain:.4f}"
        )

        if (
            round_idx >= min_round_before_switch
            and len(oracle_gain_window) == window_size
            and mean_recent_oracle_gain < oracle_negative_threshold
        ):
            negative_oracle_counter += 1
            print(
                f"Moving-average oracle gain is negative "
                f"({negative_oracle_counter}/{oracle_negative_patience})"
            )
        else:
            negative_oracle_counter = 0

        if (
            negative_oracle_counter >= oracle_negative_patience
            and not strategy_switched
        ):
            old_strategy = current_pairing_strategy
            current_pairing_strategy = switch_to
            strategy_switched = True
            negative_oracle_counter = 0

            print("\n" + "=" * 60)
            print("SWITCHING PAIRING STRATEGY")
            print(f"{old_strategy}  --->  {current_pairing_strategy}")
            print(f"Triggered at round {round_idx + 1}")
            print(f"Moving-average oracle gain = {mean_recent_oracle_gain:.4f}")
            print("=" * 60 + "\n")
