...
def make_fresh_optimizer_and_scheduler(model: nn.Module, num_steps: int = n_rounds):
    """
    Fresh optimizer/scheduler with no carried state.
    Use this only for oracle-supervised CE updates.
    """
    ...
    sch = None
    if use_scheduler:
        m1 = max(1, int(0.55 * num_steps))
        m2 = max(m1 + 1, int(0.80 * num_steps))
    ...
    for round_idx in range(n_rounds):
        ...
        for (m1, id1), (m2, id2) in pairs:
            ...
            if id1 == oracle_id or id2 == oracle_id:
                ...
                # fresh optimizer state ONLY for oracle CE update
                opt_oracle, _ = make_fresh_optimizer_and_scheduler(student, num_steps=n_rounds)
