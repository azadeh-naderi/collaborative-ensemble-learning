...
def train_student_oracle(student, train_loader, optimizer , lr_scheduler=None):
    ...
    if lr_scheduler is not None:
        lr_scheduler.step()
        # print LR after scheduler update
        current_lr = optimizer.param_groups[0]['lr']
        print(f"    [Oracle LR after step] = {current_lr:.6f}")
...
def train_student_kd(student, teacher, train_loader, optimizer , lr_scheduler=None):
    ...
    if lr_scheduler is not None:
        lr_scheduler.step()
        current_lr = optimizer.param_groups[0]['lr']
        print(f"    [KD LR after step] = {current_lr:.6f}")
...
@torch.no_grad()
def compute_ce_loss(model, data_loader):
    model.eval()
    criterion = nn.CrossEntropyLoss()
...
# to track loss
# Oracle loss tracking per round
oracle_train_loss_before_round = torch.full((n_rounds,), float('nan'), dtype=torch.float32)
oracle_train_loss_after_round  = torch.full((n_rounds,), float('nan'), dtype=torch.float32)
oracle_val_loss_before_round = torch.full((n_rounds,), float('nan'), dtype=torch.float32)
oracle_val_loss_after_round  = torch.full((n_rounds,), float('nan'), dtype=torch.float32)
oracle_train_loss_delta_round = torch.full((n_rounds,), float('nan'), dtype=torch.float32)
oracle_val_loss_delta_round   = torch.full((n_rounds,), float('nan'), dtype=torch.float32)
    ...
    for round_idx in range(n_rounds):
        ...
        for (m1, id1), (m2, id2) in pairs:
            ...
            if id1 == oracle_id or id2 == oracle_id:
                ...
                train_loss_before = compute_ce_loss(student, train_loader)
                val_loss_before   = compute_ce_loss(student, val_loader)
                ...
                train_loss_after = compute_ce_loss(student, train_loader)
                val_loss_after   = compute_ce_loss(student, val_loader)
                
                oracle_train_loss_before_round[round_idx] = train_loss_before
                oracle_train_loss_after_round[round_idx]  = train_loss_after
                
                oracle_val_loss_before_round[round_idx] = val_loss_before
                oracle_val_loss_after_round[round_idx]  = val_loss_after
                
                oracle_train_loss_delta_round[round_idx] = train_loss_after - train_loss_before
                oracle_val_loss_delta_round[round_idx]   = val_loss_after - val_loss_before
                
                print(
                    f"  oracle-gain in this round: {oracle_gain_student_var:.2f} | "
                    f"TrainLoss Δ: {train_loss_after - train_loss_before:+.4f} | "
                    f"ValLoss Δ: {val_loss_after - val_loss_before:+.4f}"
                )
...
# --------------plot loss for tarin and val
rounds = np.arange(1, n_rounds + 1)

plt.figure(figsize=(10, 5))
plt.plot(rounds, oracle_train_loss_delta_round.cpu().numpy(), label='Oracle Train Loss Δ', linewidth=2)
plt.plot(rounds, oracle_val_loss_delta_round.cpu().numpy(), label='Oracle Val Loss Δ', linewidth=2)
plt.axhline(0, linewidth=1, alpha=0.4)

plt.xlabel("Round")
plt.ylabel("Loss Change (After - Before)")
plt.title("Oracle Train/Val Loss Change per Round")
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(
    f"plot_oracle_loss_delta_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{num_runs}.png",
    dpi=200
)
plt.show()
...
axes[0].plot(rounds, oracle_train_loss_before_round.cpu().numpy(), label='Train Loss Before', linewidth=2)
axes[0].plot(rounds, oracle_train_loss_after_round.cpu().numpy(),  label='Train Loss After', linewidth=2)
axes[0].set_xlabel("Round")
axes[0].set_ylabel("CE Loss")
axes[0].set_title("Oracle Train Loss Before vs After per Round")
axes[0].legend()
axes[0].grid(True, alpha=0.3)

axes[1].plot(rounds, oracle_val_loss_before_round.cpu().numpy(), label='Val Loss Before', linewidth=2)
axes[1].plot(rounds, oracle_val_loss_after_round.cpu().numpy(),  label='Val Loss After', linewidth=2)
axes[1].set_xlabel("Round")
axes[1].set_ylabel("CE Loss")
axes[1].set_title("Oracle Val Loss Before vs After per Round")
axes[1].legend()
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(
    f"plot_oracle_loss_before_after_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{n_rounds}.png",
    dpi=200
)
plt.show()
...
plt.tight_layout()
plt.savefig(f"plot_Oracle_nonOracle_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{n_rounds}.png", dpi=200)
plt.show()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f"plot_ensAcc_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{n_rounds}.png", dpi=200)
plt.show()
...
plt.savefig(
    f"plot_per_model_accuracy_subplots_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{n_rounds}.png",
    dpi=200,
    bbox_inches='tight'
)
plt.show()
...
plt.tight_layout(rect=[0, 0, 1, 0.97])
plt.savefig(
    f"plot_per_model_og_ng_roundwise_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{n_rounds}.png",
    dpi=200,
    bbox_inches='tight'
)
plt.show()
...
plt.savefig(
    f"plot_total_og_ng_per_model_{num_models}_{model_class.__name__.lower()}_{pairing_strategy}_r{n_rounds}.png",
    dpi=200
)
plt.show()
