...
class OptimizerRegistry:
    ...
    def get(self, model_id, model, optimizer_type, learning_rate, momentum, weight_decay, use_scheduler, n_rounds, gamma):
        if model_id not in self.optimizers:
            ...
            self.optimizers[model_id] = opt
            self.schedulers[model_id] = sch

        return self.optimizers[model_id], self.schedulers[model_id]


def distill_ensemble_to_student(ensemble_models, student_model, train_loader, val_loader, device, temperature=4.0, alpha=0.7, epochs=10, lr=0.01):
